...
require(msg.sender == chairperson, "Only chairperson can add a proposal.");
...
Voter memory voter = voter_details[msg.sender];
voter.voted = true;
voter.vote = proposal;
...
Voter memory sender_details = voter_details[msg.sender];
address payable delegated_to = to;
...
Voter memory delegate_details = voter_details[delegated_to];
if (delegate_details.voted == true) {
bytes8 delegate_vote = delegate_details.vote;
